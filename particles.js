...
function createShapeBurst(points, particleFactory, rotation = 0, maxPoints) {
  const count = points.length;
  if (!count) return;

  const step =
    maxPoints && count > maxPoints ? Math.ceil(count / maxPoints) : 1;
  ...
  for (let i = 0; i < count; i += step) {
    ...
    const speedMult = Math.min(1, Math.sqrt(x * x + y * y));
    if (!speedMult) continue;
